...
if (isset($_POST['filter_by_date'])) {
    ...
    $row_user = $user->user_pending($db->conn);
}
if (isset($_POST['allow_user'])) {
    $id = $_POST["hidden"];
    $user->allow_user($id, $db->conn);
    header('Location: http://localhost/Cab_Project/admin/user_pending.php');
}
...
<div class="fiter_sort">
    <?php if (isset($_POST['filter']) || isset($_POST['filter_by_fare']) || isset($_POST['filter_by_name']) || isset($_POST['filter_by_date'])) { ?>
        <button class="filter_btn_ride">
            <a href="http://localhost/Cab_Project/admin/user_pending.php" style="text-decoration: none;">Clear</a>
        </button>
    ...
    <table class="past_ride_table" id="user">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><form method="post"><input type="hidden" name="hidden" value="<?php echo $val['user_id'];?>"><button type="submit" name="allow_user"><i class="fa fa-check" style="color:  green;"></i></button></form></td>
                <!-- <td><form method="post"><input type="hidden" name="hidden" value="<?php echo $val['user_id'];?>"><button type="submit" onClick="return confirm('Are you sure you wanna delete <?php /*echo $val['user_name'];*/?>');" name="delete"><i class='fa fa-trash' style="color:  red;"></i></button></form></td> -->
            </tr>
            <?php
            }
            ?>
