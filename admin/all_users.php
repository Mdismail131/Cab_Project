...
if (isset($_POST['filter_by_date'])) {
    ...
    $row_user = $user->all_users($db->conn);
}
if (isset($_POST['block_user'])) {
    $id = $_POST["hidden"];
    $user->block_user($id, $db->conn);
    header('Location: http://localhost/Cab_Project/admin/all_users.php');
}
...
<div class="fiter_sort">
    <?php if (isset($_POST['filter']) || isset($_POST['filter_by_fare']) || isset($_POST['filter_by_name']) || isset($_POST['filter_by_date'])) { ?>
        <button class="filter_btn_user"><a href="http://localhost/Cab_Project/admin/all_users.php" style="text-decoration: none;">Clear</a></button>
...
<div class="past_ride">
    <table class="past_ride_table" id="user">
        ...
        <tbody>
            ...
            <td>
                <form method="post">
                    <input type="hidden" name="hidden" value="<?php echo $val['user_id'];?>">
                <?php if ($val['is_block'] != '1') { ?>
                    <button type="submit" onClick="return confirm('Are you sure you want to Block <?php echo $val['user_name'];?>?')" name="block_user"><i class='fa fa-ban'></i></button>
                <?php } else {?>
                    <button type="submit" onClick="return confirm('Are you sure you want to un_block <?php echo $val['user_name'];?>?')" name="allow_user"><i class="fa fa-check" style="color:  green;"></i></button>
                <?php } ?>
                </form>
            </td>
