...
<div class="fiter_sort">
    <?php if (isset($_POST['filter']) || isset($_POST['filter_by_fare']) || isset($_POST['filter_by_name']) || isset($_POST['filter_by_date'])) { ?>
        <button class="filter_btn_ride">
            <a href="http://localhost/Cab_Project/admin/cancelled_rides.php" style="text-decoration: none;">Clear</a>
        </button>
    ...
    <table class="past_ride_table">
        ...
        <tbody>
            ...
            foreach ($row_user as $key => $val) {
                if ($val['status'] == '0' && $val['is_block'] == '1') {
            ?>
            <tr>
...
<?php } else {?>
<div>
    <h1 class="empty">Oops you delete all the data Mr.<?php echo $_SESSION['admin']?></h1>
</div>
<?php } ?>
<?php 
require "footer.php";
