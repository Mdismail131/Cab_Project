...
if (isset($_POST['filter_by_date'])) {
    ...
    $row_user = $ride->all_ride($_SESSION['user_id'], $db->conn);
}
if (isset($_POST["delete"])) {
    $id = $_POST["hidden"];
    $ride->delete($id, $db->conn);
    header('Location: http://localhost/Cab_Project/admin/total_pastride.php');
}
...
<div class="fiter_sort">
    <?php if (isset($_POST['filter']) || isset($_POST['filter_by_fare']) || isset($_POST['filter_by_date'])) { ?>
        <button id="filter_btn">
            <a href="http://localhost/Cab_Project/all_ride.php">Clear</a>
        </button>
...
<div id="previous_approved_ride" class="ride">
    <table>
        <thead>
            <tr>
                <th>Ride Id</th>
